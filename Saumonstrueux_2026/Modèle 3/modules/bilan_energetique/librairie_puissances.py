...
def P_inc_solar(lat: float, lon: float, t: float):
    ...
    lon : float
        Longitude [degrés].
    t : float
        Instant [heures depuis le 1er janvier 00:00].

    Returns
    -------
    float
        Puissance solaire incidente sur la surface horizontale [W m⁻²].
        Vaut 0 la nuit.
    """
...
def P_abs_surf_solar(lat: float, long: float, t: float, A):
    ...
    -------
    float
        Puissance solaire absorbée par la surface [W m⁻²] : (1-A) × P_inc.
    """
...
def P_em_surf_thermal(lat: float, long: float, t: float, T: float):
    ...
    long : float
        Longitude [degrés].
    t : float
        Instant [heures] (non utilisé, conservé par homogénéité d'interface).
    T : float
    ...
    -------
    float
        Puissance rayonnée par la surface (loi de Stefan-Boltzmann) [W m⁻²].
    """
    return SIGMA * (T**4)
...
def P_em_surf_conv(lat: float, long: float, t: float, T, T_air,h):
    ...
    -------
    float
        Puissance perdue par convection [W m⁻²] : h × (T - T_air).
    """
    return h * S * (T - T_air)
...
def P_abs_atm_solar(lat: float, long: float, t: float, Pinc: float):
    ...
    t : float
        Instant [heures].
    Pinc : float
        Puissance solaire incidente [W m⁻²].

    Returns
    -------
    float
        Puissance solaire absorbée par l'atmosphère [W m⁻²] (22% fixe).
    """
    AbsAtmo = 0.22
    return AbsAtmo * Pinc
...
def P_abs_atm_thermal(lat: float, long: float, t: float, T: float):
    ...
    t : float
        Instant [heures].
    T : float
        Température [K] (non utilisée, valeur fixe).

    Returns
    -------
    float
        Puissance thermique absorbée par l'atmosphère [W m⁻²] (358 W m⁻² fixe).
    """
    return 358
...
def P_em_atm_thermal_up(lat: float, long: float, t: float, alpha, P_emis):
    ...
    t : float
        Instant [heures].
    alpha : float
        Fraction du flux de surface qui s'échappe vers l'espace [-].
    P_emis : float
    ...
    -------
    float
        Puissance thermique rayonnée vers l'espace par l'atmosphère [W m⁻²].
    """
    return (alpha)*P_emis
...
def P_em_atm_thermal_down(lat: float, long: float, t: float, alpha, P_emis):
    ...
    -------
    float
        Puissance thermique re-émise vers la surface par l'atmosphère [W m⁻²]
        (effet de serre : fraction (1 - alpha) du flux de surface).
    """
