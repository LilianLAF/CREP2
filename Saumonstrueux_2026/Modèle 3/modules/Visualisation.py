...
def Visualiation(T_point, annee, lat, long):
    ...
    Parameters
    ----------
    T_point : list of float
        Série temporelle de température [K].
    annee : int
        Année simulée (utilisée pour générer les dates et le nom de fichier).
    lat : float
        Latitude du point [degrés].
    long : float
        Longitude du point [degrés].

    Returns
    -------
    None
        Sauvegarde un graphique PNG et un fichier CSV dans le répertoire courant.
    """
