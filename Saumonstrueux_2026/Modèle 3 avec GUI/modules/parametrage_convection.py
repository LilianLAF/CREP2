...
def coefficient_convection(v):
    ...
    -------
    float
        Coefficient de convection h [W m⁻² K⁻¹] calculé via la corrélation
        de Nusselt (régime laminaire ou turbulent selon Re).
    """
...
def get_daily_wind_speed(lat, lon, start="20240101", end="20241231"):
    ...
    lon : float
        Longitude [degrés].
    start : str, optional
        Date de début au format YYYYMMDD. The default is "20240101".
    end : str, optional
        Date de fin au format YYYYMMDD. The default is "20241231".

    Returns
    -------
    list of float
        Liste de 365 vitesses de vent journalières moyennes à 2 m [m s⁻¹]
        issues de l'API NASA POWER. Retourne [2.5]*365 en cas d'erreur.
    """
...
def liste_h (lat,long):
    ...
    -------
    list of float
        Liste de 8760 coefficients de convection horaires [W m⁻² K⁻¹]
        (valeur journalière répétée 24 fois).
    """
    L = []
